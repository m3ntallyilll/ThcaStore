...
## Recent Changes
- **August 5, 2025**: FIXED AUTHENTICATION SYSTEM & CART FUNCTIONALITY - Resolved login modal positioning issues (was appearing "way down there") with proper z-index and styling. Fixed cart system to allow guest users without authentication by creating anonymous guest accounts. Removed all topicals, edibles, and concentrates - now only flower (48), prerolls (55), and variety-packs (4) remain. Complete authentication flow working: login API, token verification, auth state management, and API request authorization headers all functioning correctly.
