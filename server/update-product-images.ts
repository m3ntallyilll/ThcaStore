...
import { imageGenerator } from './realistic-product-image-generator';
...
export async function updateAllProductsWithRealisticImages() {
  try {
    ...
    for (const product of products) {
      try {
        ...
        const categoryImages = realisticProductImages[product.category as keyof typeof realisticProductImages] || realisticProductImages.flower;
    ...
    console.log(`🎉 Successfully updated ${updateCount} products with realistic images!`);
    return updateCount;
...
if (require.main === module) {
  updateAllProductsWithRealisticImages()
    .then((count) => {
      console.log(`Completed! Updated ${count} products.`);
      process.exit(0);
    })
    .catch((error) => {
